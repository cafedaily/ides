# Files (generated)

- .gitattributes
- .gitignore
- AGENTS.md
- CLAUDE.md
- Makefile
- README.md
- pyproject.toml
- .clinerules/structure.md
- .cursor/rules/structure.mdc
- .github/copilot-instructions.md
- deploy/cert-renew.sh
- deploy/ides.service
- deploy/install.sh
- deploy/nginx-http.conf
- deploy/nginx-https.conf
- deploy/rollback.sh
- docs/AGENT_ASSIGNMENT_AUTH.md
- docs/AGENT_ASSIGNMENT_ROUTING.md
- docs/AGENT_ASSIGNMENT_STREAM.md
- docs/DEPLOYMENT.md
- docs/DEVELOPMENT_PLAN.md
- docs/HARNESS_FINDINGS.md
- docs/LEGACY_BACKLOG.md
- docs/LEGACY_DEBTS.md
- docs/LOCAL_SPACE.md
- docs/architecture.json
- docs/browser-validation.json
- docs/graph-benchmark.json
- docs/harness-0.2.1-validation.json
- docs/live-validation.json
- docs/local-space-validation.json
- scripts/benchmark_graph.py
- scripts/check_web.py
- test-results/browser-validation.json
- test-results/desktop.png
- test-results/local-browser-validation.json
- test-results/local-settings-dark.png
- test-results/local-settings-desktop.png
- test-results/local-settings-mobile.png
- test-results/mobile.png
- tests/browser.mjs
- tests/browser_server.py
- tests/local-unit.mjs
- tests/run.py
- tests/test_api.py
- tests/test_chat.py
- tests/test_entity.py
- tests/test_frontend.py
- tests/test_graph.py
- tests/test_incremental.py
- tests/test_jsonl.py
- tests/test_security.py
- tests/test_streaming.py
- tests/test_sync.py
- tests/test_terms.py
- tests/test_text.py
- web/build.py
- web/index.html
- web/src/01_state.js
- web/src/02_db.js
- web/src/03_api.js
- web/src/04_agent.js
- web/src/05_today.js
- web/src/06_idea.js
- web/src/07_data.js
- web/src/09_boot.js
- web/src/app.css
- web/src/body.html
- web/src/extra.css
- web/src/graph.css
- web/src/graph_view.js
- web/src/local_graph.js
- web/src/settings.css
- web/src/yangdata.js
- yang/__init__.py
- yang/__main__.py
- yang/api.py
- yang/auth.py
- yang/chat.py
- yang/cli.py
- yang/db.py
- yang/demo.py
- yang/entity.py
- yang/graph.py
- yang/graph_engine.py
- yang/jsonl.py
- yang/quality.py
- yang/semantic.py
- yang/server.py
- yang/store.py
- yang/sync.py
- yang/terms.py
- yang/text.py
